                ...
                <section className="project-banner-cyient" style={{ backgroundImage: `url(${bannerImage})` }}>
                    <div className="container">
                        <div className="banner-content">
                            <p className="breadcrumb-link"><Link to="/project">Projects</Link> / {project.name}</p>
                            <h1>{title}</h1>
                        </div>
                    </div>
                </section>
                        ...
                        <motion.div className="detail-page-gallery" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
                            ...
                            >
                                {galleryImages.map((imgSrc, index) => (
                                    <SwiperSlide key={index}><div className="gallery-image-bg" style={{ backgroundImage: `url(${imgSrc})` }}></div></SwiperSlide>
                                ))}
                            </Swiper>
                        </motion.div>

                        <motion.div className="text-center mt-5" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
                            <Link to="/contact" className="btn-modern">Discuss a Similar Project</Link>
                        </motion.div>
                    </div>
                </section>
            </div>
